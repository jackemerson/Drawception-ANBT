...
// ==/UserScript==
const wrapped = () => {
  const SCRIPT_VERSION = '1.151.2019.02'
  const NEWCANVAS_VERSION = 49 // Increase to update the cached canvas
  const SITE_VERSION = 'b87880e4' // Last seen site version
  ...
  */
  ...
  const addScriptSettings = () => {
    ...
    addGroup('Miscellaneous', [['localeTimestamp', 'boolean', `Format timestamps as your system locale (${new Date().toLocaleString()})`], ['proxyImgur', 'boolean', 'Replace imgur.com links to filmot.com to load, in case your ISP blocks them'], ['ajaxRetry', 'boolean', 'Retry failed AJAX requests'], ['autoplay', 'boolean', 'Automatically start replay when watching playback'], ['autoBypassNSFW', 'boolean', 'Automatically bypass NSFW game warning'], ['markStalePosts', 'boolean', 'Mark stale forum posts'], ['maxCommentHeight', 'number', 'Maximum comments and posts height until directly linked (px, 0 = no limit)'], ['useOldFont', 'boolean', 'Use old Nunito font (which is usually bolder and less wiggly)'], ['useOldFontSize', 'boolean', 'Use old, smaller font size'], ['markdownTools', 'boolean', 'Markdown tools for messages']])
  ...
  const pageEnhancements = () => {
    ...
    if (options.markdownTools) {
        ...
        image: {
          ...
          replaceFunc: (val, len, start, end, sel) => {
            ...
            if (sel.match(selRegex)) {
              ...
              $('#markdown-done').addEventListener('click', () => {
                const tag = `![${$('#markdown-text').value}](${$('#markdown-link').value}${$('#markdown-hover').value ? ` "${$('#markdown-hover').value}"` : ''})`
      ...
      if (textarea) {
        ...
        Object.keys(markdown).forEach(x => markdownDiv.appendChild($(`<div id="${x}" class="test-markdown fas fa-${x} btn btn-default" title="${markdown[x].title}"></div>`)))
...
localStorage.setItem(
  ...
  (
    ...
    '.btn-menu{border-color:#1e1e1e$}.btn-menu:hover{border-color:#1e1e1e$}.pagination>.active>span{color:#EEE$}#btn-notifications{color:#EEE$}.btn-warning{color:#EEE$}.alert-nice{color:#EEE$}.emotes-popup{~#2e2e2e$}.navbar-toggle{~#2e2e2e$;border-color:#1e1e1e$}.navbar-toggle .icon-bar{~#EEE$}' +
